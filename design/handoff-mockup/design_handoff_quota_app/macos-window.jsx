...
// ─────────────────────────────────────────────────────────────
function MacToolbar({ title = 'Folder', dark = false, showActions = true }) {
  return (
    <div style={{
      ...
      <div style={{
        ...
        color: dark ? 'rgba(255,255,255,0.92)' : 'rgba(0,0,0,0.85)', whiteSpace: 'nowrap', paddingLeft: 8,
      }}>{title}</div>
      <div style={{ flex: 1 }} />
      {showActions && <MacGlass dark={dark}>
        <div style={{
          width: 36, height: 36, display: 'flex',
          alignItems: 'center', justifyContent: 'center',
        }}>
          <div style={{ width: 14, height: 14, borderRadius: '50%', background: dark ? '#fff' : '#4c4c4c', opacity: 0.4 }} />
        </div>
      </MacGlass>}
      {showActions && <MacGlass dark={dark}>
        <div style={{
          ...
          <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
            <circle cx="5.5" cy="5.5" r="4" stroke={dark ? '#9a9a9e' : '#727272'} strokeWidth="1.5"/>
            <path d="M8.5 8.5l3 3" stroke={dark ? '#9a9a9e' : '#727272'} strokeWidth="1.5" strokeLinecap="round"/>
          </svg>
          <span style={{
            fontFamily: MAC_FONT, fontSize: 13, fontWeight: 500, color: dark ? '#9a9a9e' : '#727272',
          }}>Search</span>
        </div>
      </MacGlass>}
    </div>
...
// ─────────────────────────────────────────────────────────────
function MacSidebarItem({ label, selected = false, dark = false, icon, accent = '#007aff' }) {
  return (
    <div style={{
      display: 'flex', alignItems: 'center', gap: 8,
      height: 28, padding: '4px 10px', margin: '0 10px',
      borderRadius: 8, position: 'relative',
      fontFamily: MAC_FONT, fontSize: 12.5, fontWeight: 500,
    }}>
      {selected && (
        <div style={{
          position: 'absolute', inset: 0, borderRadius: 8,
          background: dark ? 'rgba(255,255,255,0.12)' : 'rgba(0,0,0,0.11)',
          mixBlendMode: dark ? 'normal' : 'multiply',
        }} />
      )}
      {icon ? (
        <div style={{ width: 16, height: 16, flexShrink: 0, position: 'relative', color: selected ? accent : (dark ? 'rgba(255,255,255,0.55)' : 'rgba(0,0,0,0.55)'), display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{icon}</div>
      ) : (
        <div style={{
          width: 14, height: 14, borderRadius: '50%',
          background: selected ? accent : (dark ? '#fff' : 'rgba(0,0,0,0.4)'),
          opacity: selected ? 1 : 0.5, flexShrink: 0, position: 'relative',
        }} />
      )}
      <span style={{ color: selected ? (dark ? '#fff' : '#000') : (dark ? 'rgba(255,255,255,0.85)' : 'rgba(0,0,0,0.85)'), position: 'relative' }}>{label}</span>
    </div>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: dark ? 'rgba(255,255,255,0.055)' : 'rgba(210,225,245,0.45)',
        backdropFilter: 'blur(50px) saturate(200%)',
        WebkitBackdropFilter: 'blur(50px) saturate(200%)',
        border: dark ? '0.5px solid rgba(255,255,255,0.09)' : '0.5px solid rgba(255,255,255,0.5)',
        boxShadow: dark ? '0 8px 40px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.05)' : '0 8px 40px rgba(0,0,0,0.10), inset 0 1px 0 rgba(255,255,255,0.35)',
      }} />
      {/* content */}
      <div style={{
...
function MacSidebarHeader({ title, dark = false }) {
  return (
    <div style={{
...
function MacWindow({
  ...
  sidebar, children, dark = false, noSidebar = false, showToolbarActions = true,
}) {
  return (
    ...
    <div data-om-starter="macos-window" style={{
      ...
      {!noSidebar && <MacSidebar dark={dark}>{sidebar}</MacSidebar>}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        {!noSidebar && <MacToolbar title={title} dark={dark} showActions={showToolbarActions} />}
        {noSidebar && <div style={{ padding: '14px 16px 0' }}><MacTrafficLights /></div>}
        <div style={{ flex: 1, overflow: 'auto', padding: noSidebar ? 0 : '4px 8px' }}>
          {children}
        </div>
      </div>
    </div>
